...
ncclRedOp_t test_ops[] = {
    ncclSum, ncclProd, ncclMax, ncclMin
...
void Allreduce(struct threadArgs *args, T *value, int average)
{
  ...
  if (args->thread + 1 == args->nThreads)
  {
    ...
    if (average != 0)
    {
      ...
      MPI_Datatype ty = std::is_same<T, long long>::value ? MPI_LONG_LONG : std::is_same<T, double>::value ? MPI_DOUBLE
                                                                                                           : MPI_Datatype();
      MPI_Op op = average == 1 ? MPI_SUM : average == 2 ? MPI_MIN
                                       : average == 3   ? MPI_MAX
                                       : average == 4   ? MPI_SUM
                                                        : MPI_Op();
