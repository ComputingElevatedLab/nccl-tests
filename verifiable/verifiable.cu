...
namespace
{
  ...
  template <typename T, typename ReduceFn,
            typename = typename std::enable_if<
                !std::is_same<ReduceFn, ReduceNil>::value>::type>
...
#define CASE_TY(T)                                                                                  \
  prepareInput2<<<block_n, 512, 0, stream>>>((T *)elts, elt_n, op, rank_n, rank_me, seed, elt_ix0); \
  break;
    switch (elt_ty)
    {
    case ncclInt8:
...
#if HAVE_ncclAvg
  case ncclAvg:
    CASE_OP(ReduceAvg{rank_n})
#endif
#if HAVE_ncclPreMulSum
  default:
    CASE_OP(ReducePreMulSum())
#endif
  }
#undef CASE_OP
}
...
namespace
{
      ...
      uint64_t seed, intptr_t elt_ix0, cudaStream_t stream)
  {
    int block_n = std::min<intptr_t>(32, (elt_n + 4 * 512 - 1) / (4 * 512));
#define CASE_TY(T)                                                                            \
  prepareExpected2<<<block_n, 512, 0, stream>>>((T *)elts, elt_n, op, rank_n, seed, elt_ix0); \
  break;
    switch (elt_ty)
    {
    case ncclInt8:
...
namespace
{
  ...
  __host__ __device__ uint64_t calcDelta(T a, T b)
  {
    ...
    {
    case 1:
      return x.i1 < y.i1 ? y.i1 - x.i1 : x.i1 - y.i1;
    case 2:
      return x.i2 < y.i2 ? y.i2 - x.i2 : x.i2 - y.i2;
    case 4:
      return x.i4 < y.i4 ? y.i4 - x.i4 : x.i4 - y.i4;
    default:
      return x.i8 < y.i8 ? y.i8 - x.i8 : x.i8 - y.i8;
    }
  }
}
...
#define CASE_OP(op)                                                                                                                           \
  if (rank_n == 1)                                                                                                                            \
    verifyInline2<T, Uint><<<block_n, 512, 0, stream>>>((T const *)results, elt_n, ReduceNil(), rank_n, seed, elt_ix0, tolerance, bad_elt_n); \
  else                                                                                                                                        \
    verifyInline2<T, Uint><<<block_n, 512, 0, stream>>>((T const *)results, elt_n, op, rank_n, seed, elt_ix0, tolerance, bad_elt_n);          \
  break;
    switch (red_op)
    {
    case ncclSum:
...
#define CASE_TY(T, Uint)                                                                                                       \
  ...
  {
  case ncclInt8:
    CASE_TY(int8_t, uint8_t)
  case ncclUint8:
    CASE_TY(uint8_t, uint8_t)
  case ncclInt32:
    CASE_TY(int32_t, uint32_t)
  case ncclUint32:
    CASE_TY(uint32_t, uint32_t)
  case ncclInt64:
    CASE_TY(int64_t, uint64_t)
  case ncclUint64:
    CASE_TY(uint64_t, uint64_t)
  case ncclFloat16:
    CASE_TY(half, uint16_t)
#if HAVE_ncclBfloat16
  case ncclBfloat16:
    CASE_TY(__nv_bfloat16, uint16_t)
#endif
  case ncclFloat32:
    CASE_TY(float, uint32_t)
  case ncclFloat64:
